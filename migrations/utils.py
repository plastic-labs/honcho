...
def table_exists(
    table_name: str, inspector: Optional[sa.Inspector] = None
) -> bool:
...
def check_constraint_exists(
    table_name: str, constraint_name: str, inspector: Optional[sa.Inspector] = None
) -> bool:
...
def primary_constraint_exists(
    table_name: str, primary_key_name: str, inspector: Optional[sa.Inspector] = None
) -> bool:
    """Check if a primary key constraint exists in a table."""
    if inspector is None:
        inspector = sa.inspect(op.get_bind())
    schema = get_schema()
    constraint = inspector.get_pk_constraint(table_name, schema=schema)
    return constraint["name"] == primary_key_name