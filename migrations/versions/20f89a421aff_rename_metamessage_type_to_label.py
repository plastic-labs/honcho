...
def upgrade() -> None:
    ...
    op.alter_column(
        "metamessages", "metamessage_type", new_column_name="label", existing_type=sa.TEXT()
    )
    ...
    # NOTE: The actual name in the database might differ (e.g. ck_metamessages_metamessage_type_length)
    # If this fails, you might need to inspect the database for the correct constraint name.
    op.execute("ALTER TABLE metamessages DROP CONSTRAINT metamessage_type_length;")
    op.create_check_constraint(
        "label_length", "metamessages", "length(label) <= 512"
    )
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.alter_column(
        "metamessages", "label", new_column_name="metamessage_type", existing_type=sa.TEXT()
    )
