"""add queue_item_batches aggregate for the deriver claim
...
# - _queue_item_batches_recompute LOCKS THE BACKLOG ROW FIRST (FOR UPDATE) and
...
#   subquery; a live unit would lose its batch row permanently. Taking the
...
def upgrade() -> None:
    if not table_exists("queue_item_batches"):
        op.create_table(
            "queue_item_batches",
    ...
    op.execute(
        f"""
        CREATE OR REPLACE FUNCTION {schema}._queue_item_batches_recompute(
            key TEXT
        ) RETURNS void AS $$
        DECLARE
            upserted INTEGER;
        BEGIN
            ...
            PERFORM 1 FROM {schema}.queue_item_batches
            WHERE work_unit_key = key FOR UPDATE;

            INSERT INTO {schema}.queue_item_batches AS wub
            ...
            DELETE FROM {schema}.queue_item_batches
    ...
    op.execute(
        f"""
        CREATE OR REPLACE FUNCTION {schema}.queue_item_batches_apply_insert()
        RETURNS trigger AS $$
        BEGIN
    ...
    op.execute(
        f"""
        CREATE OR REPLACE FUNCTION {schema}.queue_item_batches_apply_update()
        RETURNS trigger AS $$
        DECLARE
            affected_key TEXT;
        BEGIN
            FOR affected_key IN
                ...
                ORDER BY 1
            LOOP
                PERFORM {schema}._queue_item_batches_recompute(affected_key);
            END LOOP;
            RETURN NULL;
        END $$ LANGUAGE plpgsql;
        """
    ...
    op.execute(
        f"""
        CREATE OR REPLACE FUNCTION {schema}.queue_item_batches_apply_delete()
        RETURNS trigger AS $$
        DECLARE
            affected_key TEXT;
        BEGIN
    ...
    op.execute(
        f"""
        DROP TRIGGER IF EXISTS trg_queue_item_batches ON {schema}.queue;
        DROP TRIGGER IF EXISTS trg_queue_item_batches_insert ON {schema}.queue;
        DROP TRIGGER IF EXISTS trg_queue_item_batches_update ON {schema}.queue;
        DROP TRIGGER IF EXISTS trg_queue_item_batches_delete ON {schema}.queue;
        CREATE TRIGGER trg_queue_item_batches_insert
        AFTER INSERT ON {schema}.queue
        FOR EACH ROW EXECUTE FUNCTION {schema}.queue_item_batches_apply_insert();
        CREATE TRIGGER trg_queue_item_batches_update
        AFTER UPDATE ON {schema}.queue
        REFERENCING OLD TABLE AS old_rows NEW TABLE AS new_rows
        FOR EACH STATEMENT
        EXECUTE FUNCTION {schema}.queue_item_batches_apply_update();
        CREATE TRIGGER trg_queue_item_batches_delete
        AFTER DELETE ON {schema}.queue
        REFERENCING OLD TABLE AS old_rows
        FOR EACH STATEMENT
        EXECUTE FUNCTION {schema}.queue_item_batches_apply_delete();
        """
    )
...
def downgrade() -> None:
    op.execute(
        f"DROP TRIGGER IF EXISTS trg_queue_item_batches_insert ON {schema}.queue"
    )
    op.execute(
        f"DROP TRIGGER IF EXISTS trg_queue_item_batches_update ON {schema}.queue"
    )
    op.execute(
        f"DROP TRIGGER IF EXISTS trg_queue_item_batches_delete ON {schema}.queue"
    )
    op.execute(f"DROP FUNCTION IF EXISTS {schema}.queue_item_batches_apply_insert()")
    op.execute(f"DROP FUNCTION IF EXISTS {schema}.queue_item_batches_apply_update()")
    op.execute(f"DROP FUNCTION IF EXISTS {schema}.queue_item_batches_apply_delete()")
    op.execute(f"DROP FUNCTION IF EXISTS {schema}._queue_item_batches_recompute(TEXT)")
    op.execute(
        f"DROP FUNCTION IF EXISTS {schema}._queue_item_token_count(TEXT, BIGINT, TEXT)"
    )
    if table_exists("queue_item_batches"):
        op.drop_table("queue_item_batches", schema=schema)
