...
#   later snapshot, so a live unit never loses its batch row. pending_count
...
def upgrade() -> None:
    ...
    if not index_exists("queue_item_batches", "ix_queue_item_batches_tenant_id"):
        op.create_index(
            "ix_queue_item_batches_tenant_id",
            "queue_item_batches",
            ["tenant_id"],
            schema=schema,
        )
    ...
    op.execute(
        f"""
        CREATE OR REPLACE FUNCTION {schema}.queue_item_batches_apply()
        RETURNS trigger AS $$
        BEGIN
            ...
            ELSIF TG_OP = 'UPDATE' THEN
                IF OLD.processed IS DISTINCT FROM NEW.processed THEN
                    PERFORM {schema}._queue_item_batches_recompute(
                        NEW.work_unit_key
                    );
                END IF;
    ...
    op.execute(
        ...
        CREATE TRIGGER trg_queue_item_batches
        AFTER INSERT OR UPDATE OF processed OR DELETE ON {schema}.queue
        FOR EACH ROW EXECUTE FUNCTION {schema}.queue_item_batches_apply();
        """
    )
...
def downgrade() -> None:
    op.execute(f"DROP TRIGGER IF EXISTS trg_queue_item_batches ON {schema}.queue")
    op.execute(f"DROP FUNCTION IF EXISTS {schema}.queue_item_batches_apply()")
