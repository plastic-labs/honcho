...
class AsyncMessage(BaseModel):
    """
    Represents a message in Honcho with async operations.

    Messages are scoped to sessions and provide full message data plus async SDK methods.
    ...
    @classmethod
    def from_core(
        cls, core_message: MessageCore, client: AsyncHonchoCore
    ) -> "AsyncMessage":
        """
        Create async SDK Message from core Message.

        This is the primary way AsyncMessage objects are created from API responses.
        ...
        Returns:
            Async SDK Message object with all data and methods available
        """
        return cls(
    ...
    @validate_call
    async def update(self, metadata: dict[str, Any]) -> Any:
        """
        Update metadata for this message.

        Makes an async API call to update the metadata associated with this message.
        ...
        Returns:
            The updated AsyncMessage object (self)
        """
        await self._client.workspaces.sessions.messages.update(
            message_id=self.id,
    ...
    def __repr__(self) -> str:
        """
        Return a string representation of the AsyncMessage.

        Returns:
            A string representation suitable for debugging
        """
        return f"AsyncMessage(id='{self.id}', session_id='{self.session_id}', workspace_id='{self.workspace_id}')"

    def __str__(self) -> str:
        """
        Return a human-readable string representation of the AsyncMessage.
