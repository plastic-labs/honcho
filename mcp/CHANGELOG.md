...
- `source_message_ids` on every conclusion the tools return: the messages an explicit conclusion cites as evidence (Honcho v3.3.0+, null for derived levels and on older servers). `list_conclusions` filters accept `{"source_message_ids": {"contains": "<message id>"}}` to find the conclusions that cite a message as evidence. Surfaces once `@honcho-ai/sdk` carries the field; earlier SDKs report null
