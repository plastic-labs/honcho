...
By default, all sessions have the deriver enabled, much like peers. You may create a session that escapes the deriver's watchful eye by setting the `deriver_enabled` flag to `False`. You can update the flag by calling `get_or_create` on the session with a new configuration.
...
session = honcho.session("my-session", config={"deriver_enabled": False})
```
```typescript TypeScript
import { Honcho } from "@honcho-ai/sdk";
...
(async () => {
    ...
    // Create session with configuration
    const session = await honcho.session("my-session", { config: { deriver_enabled: false } });
})();
```
</CodeGroup>
