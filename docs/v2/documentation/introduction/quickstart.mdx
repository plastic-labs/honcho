...
/**
 * Load a conversation JSON export, import it into Honcho, then ask for a short
 * set of user insights derived from that history.
 *
 * This is intentionally wrapped in `main()` (no top-level await) so it works
 * with the default CommonJS `ts-node` setup.
 */
