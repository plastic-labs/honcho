...
description: 'Error handling strategies in Honcho'
icon: 'bug'
---
...
```python
@router.get("/{user_id}")
async def get_user(app_id: str, user_id: str, db=db):
...
```python
@router.post("")
async def create_user(app_id: str, user: schemas.UserCreate, db=db):
    try:
