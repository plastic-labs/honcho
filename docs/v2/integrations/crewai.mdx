...
storage = HonchoStorage(
    ...
    session_id=None,               # Optional: Specific `session` ID (auto-generated if None)
...
- **`search()`** - Performs semantic vector search using `session.search()` to find messages most relevant to the query.
...
- **`HonchoDialecticTool`** - Queries theory-of-mind representations about users. Use for understanding user preferences and characteristics without full message history.
- **`HonchoSearchTool`** - Performs semantic search for specific information. Use for targeted queries like "what did the user say about budget?"
...
You can combine both: automatic memory for baseline context, tools for specific queries.
...
- **Leverage theory-of-mind** via the Dialectic API for deep user understanding
...
<CardGroup cols={2}>
  ...
  <Card title="Dialectic API" icon="brain" href="/v2/documentation/core-concepts/features/dialectic">
    Query theory-of-mind representations for deep user understanding
  </Card>
