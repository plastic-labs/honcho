...
<Note>
Before proceeding, it's important to understand Honcho's core concepts (Peers, Sessions, and Messages). Review the [Honcho Architecture](/v2/documentation/core-concepts/architecture) to familiarize yourself with these primitives.
</Note>
...
class State(TypedDict):
    ...
    user: object  # Honcho Peer object
    assistant: object  # Honcho Peer object
    session: object  # Honcho Session object
```
...
The [`get_context()`](/v2/documentation/core-concepts/features/get-context) method retrieves the conversation history and formats it for your LLM. It automatically:
...
The `run_conversation_turn` function initializes Honcho peers and a session, passes them to the LangGraph, and returns the assistant's response. By calling it repeatedly with the same `user_id` and in the same session, the chat builds context over time.

<Note>
In production, you'd get the `user_id` from authentication and `session_id` from session management. This tutorial uses a hardcoded test user for simplicity.
</Note>
...
- **Create tools with Honcho** to fully utilize Honcho's memory & context management features
