...
When n8n refers to "memory" in AI nodes, they typically mean vector databases for RAG-style retrieval. Honcho is more then a database. It offers richer context and reasoning than similarity search alone. Read more about our approach [here](https://blog.plasticlabs.ai/blog/Memory-as-Reasoning).
...
As you build the data ingestion workflow, verify everything is created correctly in your [Honcho instance](app.honcho.dev/).
</Tip>
...
```json
{
  ...
  "nodes": [
    ...
    {
      "parameters": {
        "url": "=https://api.honcho.dev/v2/workspaces/(workspace)peers/{peer}/context",
        "authentication": "predefinedCredentialType",
        "nodeCredentialType": "httpBearerAuth",
        "options": {}
      },
    ...
    },
    {
      "parameters": {
        "method": "POST",
        "url": "https://api.honcho.dev/v2/workspaces/{workspace}/peers/{peer}/chat",
        "authentication": "predefinedCredentialType",
        "nodeCredentialType": "httpBearerAuth",
        "sendBody": true,
        "bodyParameters": {
