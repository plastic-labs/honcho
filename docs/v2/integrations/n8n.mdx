...
[Download workflow JSON from GitHub](https://github.com/plastic-labs/honcho/blob/main/examples/n8n/n8n.json) or expand below to copy:
  ...
  "nodes": [
    ...
    },
    {
      "parameters": {
        ...
        "sendBody": true,
        "specifyBody": "json",
        "jsonBody": "={\"messages\": [{\"content\": \"{{ $('Get a message').item.json.snippet }}\", \"peer_id\": \"{{ $('Clean Names').first().json.name }}\"}]}",
        "options": {}
      },
      "type": "n8n-nodes-base.httpRequest",
      "typeVersion": 4.3,
...
The tutorial uses hardcoded workspace (`email-test`) and session (`new_session`) IDs for simplicity. For production:

- **Dynamic IDs**: Use n8n variables or pass IDs through the workflow instead of hardcoding them
- **Add Dialectic Chat**: Add another HTTP Request Tool with `POST /workspaces/{id}/dialectic/chat` to enable natural language queries about peers. See the [Dialectic API docs](https://docs.honcho.dev/v2/documentation/core-concepts/features/dialectic) for details.
- **Multiple sessions**: Create unique session IDs per conversation thread or user
