...
**Advanced Multi-Peer Sessions** - Honcho offers configurable observation settings (who build's memories about whom), representation based queries between participants, and first-class peer objects.
...
| **Search** | `client.search(query, user_id=...)` | `peer.search(query)` or `session.search(query)` | Scoped to peer or session |
| **List all** | `client.get_all(user_id=...)` | `session.get_messages()` | Paginated results |
...
Mem0's `search()` returns basic vector or semantic or raw memory matches. Honcho's `peer.chat()` enables your agent to *reason* about what it knows—returning synthesized natural language insights with streaming support and scoped queries.
...
### Multi-Peer / Group Chat Comparison

Both platforms support multi-participant conversations, but with different approaches:

| Capability | Mem0 | Honcho |
|------------|------|--------|
| **Participant identity** | `name` field in messages | First-class `Peer` objects |
| **Attribution** | Auto-extracts to `user_id`/`agent_id` | Explicit `peer.message()` attribution |
| **Session grouping** | `run_id` parameter | `session` object with `add_peers()` |
| **Observation control** | None | `SessionPeerConfig` (who learns about whom) |
| **Inference powered queries** | Not supported | `peer.chat(query, target=other_peer)` |
| **Peer relationships** | Flat attribution | Bidirectional representation building |
