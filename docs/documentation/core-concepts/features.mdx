...
- **Local Representation**: Session-level representation that Peers can optionally form of other Peers in a Session, based on messages that the Peer has seen.
...
- If a session is specified, the queue status reflects pending work for the Peer's local representation queue.
