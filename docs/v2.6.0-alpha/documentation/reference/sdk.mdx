...
The `get_context()` method on peers retrieves both the representation and peer card in a single API call:
...
// Get peer's own context
const context = await alice.getContext();
console.log(context.representation);
...
# Representation queries with semantic search
