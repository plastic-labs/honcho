...
model-agnostic, allowing developers to build agents with any LLM while using
Honcho's primitives, and highly composable with other agent frameworks.
...
are stored as vector embeddings to allow for a RAG like interface. As a result,
Honcho allows developers to query document collections using cosine similarity
search, semantic search, and more.
