...
```python Python
honcho.apps.users.sessions.messages.create(session.id, app.id, user.id, content="User message", is_user=True)
honcho.apps.users.sessions.messages.create(session.id, app.id, user.id, content="AI message", is_user=False)
```

```javascript NodeJS
honcho.apps.users.sessions.messages.create(app.id, user.id, session.id, { content: "User message", is_user: true })
honcho.apps.users.sessions.messages.create(app.id, user.id, session.id, { content: "AI message", is_user: false })
```
