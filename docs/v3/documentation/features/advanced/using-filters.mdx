...
<Warning>
The shorthand applies to regular columns only. Inside `metadata`, a bare list
keeps JSONB containment semantics — `{"metadata": {"tags": ["a", "b"]}}` matches
records whose `tags` array *contains* both values, it does not mean "`tags` is
either `a` or `b`". Use the explicit `{"in": [...]}` form when you want
membership on a metadata field.
</Warning>
...
a **session allowlist**, restricting everything the request can recall
(conclusions *and* messages) to the sessions you name.
...
<Note>
On the chat endpoint, a peer-scoped key must be an active member of every session
it names, and the request is rejected with `401` otherwise — the allowlist reaches
message recall and session history there, so it could otherwise read a session the
peer never joined.

The representation endpoint does not run that membership check. It doesn't need
to: key scope already confines the caller to its own peer's representation, and an
allowlist can only narrow what that representation already contains. Workspace and
admin keys take the allowlist as given on both endpoints.
</Note>
