...
**Workspace Chat:** Query knowledge across all peers in the workspace rather than about a single peer.

<CodeGroup>
```python Python
# Query knowledge across ALL peers in the workspace
response = honcho.chat("What are common themes across all users?")

# With reasoning level
response = honcho.chat("Summarize all peer activity.", reasoning_level="high")

# Streaming
stream = honcho.chat_stream("What do all users have in common?")
for chunk in stream:
    print(chunk, end="", flush=True)

# Async
response = await honcho.aio.chat("Which users discussed topic X?")
stream = await honcho.aio.chat_stream("Summarize the workspace.")
...
// With reasoning level
const detailed = await honcho.chat("Summarize all peer activity.", {
  reasoningLevel: "high"
});

// Streaming
const stream = await honcho.chatStream("What do all users have in common?");
