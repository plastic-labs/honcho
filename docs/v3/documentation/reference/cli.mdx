...
`honcho start` runs a personal Honcho server on your machine via Docker (API, deriver, Postgres, Redis) using cloud LLM inference. Files live under `~/.honcho/profiles/local/`.
