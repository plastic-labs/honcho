...
|----------|---------------|
| How many workspaces? | One per application. Separate per-tenant if you need hard data isolation. |
...
<CodeGroup>
```python Python
from honcho import Honcho, PeerConfig
...
- **Not storing messages** -- Honcho reasons about messages asynchronously. If you don't call `add_messages()`, there's nothing to reason about — no messages means no memory. See [Storing Data](/v3/documentation/features/advanced/storing-data) for details.
...
- **Too many tiny sessions** -- Summaries and `get_context` are scoped to a single session. If you split a continuous conversation across many sessions, context is fragmented and each session is too short to summarize. Reuse a session when context should flow continuously.
