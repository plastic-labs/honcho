...
|----------|---------------|
| How many workspaces? | One per application. Separate per-tenant if you need hard isolation. |
| Who should be a peer? | Any entity you want to reason about. Users, agents, NPCs, students, customers. |
| Should I set `observe_me: false`? | Yes, for any peer your application fully controls (assistants, system bots). |
| How should I scope sessions? | Per-conversation for chatbots, per-channel for persistent threads, per-scene for games. |
| Do I need `observe_others`? | Only when different peers need different views of the same entity (games, multi-agent). Default off is fine for most apps. |
| How do I import external data? | Create a single-peer session and structure documents/emails as messages. |
...
Once a `Message` is saved in Honcho, it kicks off a background task that
reasons about the new data to generate insights about the `Peer` that sent it.

This is the default behavior and can be turned off by [configuring the
Peer or Session](/v3/documentation/features/advanced/reasoning-configuration).

The pattern of Peer + Session + Messages is highly flexible. The rest of this
guide shows how to model these primitives for common application types.
...
<Info>
You don't need to create workspaces explicitly. The SDK creates one called `default` if you don't specify a `workspace_id`
</Info>
...
A peer is any entity you want Honcho to build a representation of over time. The key question is: **who or what should Honcho reason about?**

**What makes a good peer?**

- It persists across sessions (a user, an agent, a character)
- It changes over time (preferences shift, knowledge grows)
- You want to query insights about it later
...
Not every peer needs a representation. Set `observe_me: false` on peers that your application controls and doesn't need insights about:

<CodeGroup>
```python Python
# The assistant doesn't need a representation--you control its behavior
assistant = honcho.peer("assistant", configuration={"observe_me": False})
...
- Honcho automatically summarizes long sessions, so don't worry about sessions getting too large
...
agent = honcho.peer("agent-main", configuration={"observe_me": False})

# Session key = thread + platform → separate histories, shared user memory
session = honcho.session("general-discord")
session.add_peers([
    (owner, {"observe_me": True, "observe_others": False}),
    (agent, {"observe_me": True, "observe_others": True}),
])
...
await session.addPeers([
    ...
    [agent.id, { observeMe: true, observeOthers: true }],
]);
...
- **Asymmetric observation** — owner is observed; agent stores messages so Honcho sees both sides but has reasoning disabled. Subagents get lighter context (peer card only, no session summary)
...
developer = honcho.peer("ajspig")
agent = honcho.peer("claude", configuration={"observe_me": False})

# Session per project directory -- stable across restarts
session = honcho.session("ajspig-honcho")
...
const developer = await honcho.peer("ajspig");
const agent = await honcho.peer("claude", { configuration: { observeMe: false } });

const session = await honcho.session("ajspig-honcho");
...
- **Session-per-directory** by default -- each project accumulates its own memory. Prefix with peer name (`ajspig-honcho`) so multiple developers on the same workspace don't collide. Alternative strategies: `git-branch` (session switches on branch change) or `chat-instance` (clean slate each time)
...
- **Forgetting to add peers to a session** -- Messages require their author peer to be part of the session.
- **Leaving `observe_me` on for assistants** -- Wastes reasoning compute on a peer you control. Disable it.
...
- **Too many tiny sessions** -- Each session triggers its own summarization. If context should flow continuously, reuse the session.
