...
The `.env.template` has transport and model lines ready for each feature. After copying it to `.env`, you need to set two things:

```bash
# 1. Your API key (already uncommented in the template)
LLM_OPENAI_API_KEY=sk-or-v1-...

# 2. Replace "your-model-here" everywhere with your model and set
#    OVERRIDES__BASE_URL on each feature to point at your endpoint
...
# ... same for summary, dream, and each dialectic level
```
