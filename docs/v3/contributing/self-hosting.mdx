...
The default image also excludes Milvus. To use Milvus with Docker Compose, configure `VECTOR_STORE_TYPE=milvus`, set `VECTOR_STORE_MILVUS_URI` to a remote Milvus Server or Zilliz Cloud endpoint (and set `VECTOR_STORE_MILVUS_TOKEN` for Cloud), then run `INSTALL_MILVUS=true docker compose up -d --build`. Milvus Lite is not supported in this two-container deployment: API reads and deriver writes happen in separate processes, so a local database file would not be shared or durable. The compose configuration fails at startup if a local Lite URI is selected.
