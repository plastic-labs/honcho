...
If you keep the built-in defaults, you only need one API key: all text-generation features default to `openai / gpt-5.4-mini`, and embeddings default to `openai / text-embedding-3-small`. Any OpenAI-compatible endpoint works too — OpenRouter, Together, Fireworks, Ollama, vLLM, LiteLLM, or OrcaRouter. Models must support tool calling (function calling).
...
Or use the named `orcarouter` transport to route through [OrcaRouter](https://www.orcarouter.ai) (an OpenAI-compatible gateway) without spelling out a base URL — it defaults to `https://api.orcarouter.ai/v1` and reads `LLM_ORCAROUTER_API_KEY`:

```bash
LLM_ORCAROUTER_API_KEY=sk-orca-...

DERIVER_MODEL_CONFIG__TRANSPORT=orcarouter
DERIVER_MODEL_CONFIG__MODEL=auto
```
