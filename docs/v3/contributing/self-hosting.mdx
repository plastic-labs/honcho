...
You need one API key and one model. Any OpenAI-compatible endpoint works — OpenRouter, Together, Fireworks, Ollama, vLLM, or a direct vendor API. The `.env.template` has provider and model lines ready for each feature — just set your endpoint, API key, and replace `your-model-here` with any model that supports tool calling.
...
Migrations run automatically on startup. Check that services are running with `docker compose ps`, then see [Verify Your Setup](#verify-your-setup) below.
