...
For text generation, you can use a ChatGPT Codex subscription with no OpenAI API key by setting `*_MODEL_CONFIG__OVERRIDES__AUTH_MODE=codex_oauth` on those same text-generation sections. Honcho uses Codex CLI OAuth tokens from `$CODEX_HOME/auth.json` or `~/.codex/auth.json`. Embeddings still require a configured embedding provider key.
