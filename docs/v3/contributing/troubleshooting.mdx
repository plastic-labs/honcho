...
**Fix:** Set `THINKING_BUDGET_TOKENS=0` for every component when using models that don't support thinking:
...
For OpenAI reasoning models, use `MODEL_CONFIG__THINKING_EFFORT` instead of `THINKING_BUDGET_TOKENS`.
