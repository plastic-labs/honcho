...
| `orcarouter` | [OrcaRouter](https://www.orcarouter.ai) — an OpenAI-compatible gateway | `LLM_ORCAROUTER_API_KEY` |

For OpenAI-compatible proxies (OpenRouter, vLLM, Ollama, OrcaRouter, etc.), use `transport = "openai"` and set `MODEL_CONFIG__OVERRIDES__BASE_URL` on each feature to point at your endpoint. The `orcarouter` transport is a named variant that defaults to `https://api.orcarouter.ai/v1` and reads `LLM_ORCAROUTER_API_KEY`, so you can route any feature through OrcaRouter without spelling out the base URL.
