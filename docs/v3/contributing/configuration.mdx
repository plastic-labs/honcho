...
<Note>
Some OpenAI-compatible providers don't support OpenAI Structured Outputs (`json_schema`). Set `STRUCTURED_OUTPUT_MODE=json_object` on the relevant model config (e.g. `DERIVER_MODEL_CONFIG__STRUCTURED_OUTPUT_MODE=json_object`) to request loose JSON mode and inject the schema into the prompt instead.
</Note>
