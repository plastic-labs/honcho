...
`[embedding.model_config.overrides.provider_params]` accepts `timeout` too
(OpenAI-compatible and Gemini transports both). The Gemini embedding client
already carried a fixed 10-minute HTTP timeout for this reason — a stalled
socket otherwise wedges the deriver worker's event loop, since the reconciler
runs in-process. `timeout` makes that configurable and extends the same
protection to OpenAI-compatible embedding backends (self-hosted or otherwise),
...
- The value is validated **at config load**: it must coerce to a positive,
  ...
  This applies to the primary model config, its `fallback.overrides`, and
