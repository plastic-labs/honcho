...
- `[llm]` - LLM provider API keys, OpenAI-compatible endpoint defaults, and general LLM settings
...
- `DERIVER_MODEL_CONFIG__MODEL` → `[deriver.model_config].model`
- `LOG_LEVEL` (no section) → `[app].LOG_LEVEL`
- `DIALECTIC_LEVELS__minimal__MODEL_CONFIG__MODEL` → `[dialectic.levels.minimal.model_config].model`
...
# Optional default base URLs for transport-compatible proxies
LLM_ANTHROPIC_BASE_URL=https://your-anthropic-compatible-endpoint.com
LLM_OPENAI_BASE_URL=https://your-openai-compatible-endpoint.com
LLM_GEMINI_BASE_URL=https://your-gemini-compatible-endpoint.com
LLM_GROQ_BASE_URL=https://your-groq-compatible-endpoint.com
```

`MODEL_CONFIG.transport` now identifies the API contract directly, and `MODEL_CONFIG.model` is the raw model id passed to that transport. If you want to point a transport at a different endpoint, override `base_url` and `api_key` at the module level.

For example, to use an OpenAI-compatible local endpoint:

- `MODEL_CONFIG.transport = "openai"`
- `MODEL_CONFIG.model = "my-local-model"`
- `MODEL_CONFIG.overrides.base_url` for the target endpoint
- `MODEL_CONFIG.overrides.api_key_env` if the module needs its own credentials
...
DERIVER_MODEL_CONFIG__TEMPERATURE=  # Optional temperature override (unset by default)

# Backup model (optional)
# DERIVER_MODEL_CONFIG__FALLBACK_MODEL=claude-haiku-4-5
# DERIVER_MODEL_CONFIG__FALLBACK_TRANSPORT=anthropic
...
# Backup model (optional)
# SUMMARY_MODEL_CONFIG__FALLBACK_MODEL=claude-haiku-4-5
# SUMMARY_MODEL_CONFIG__FALLBACK_TRANSPORT=anthropic
...
DREAM_MODEL_CONFIG__MAX_OUTPUT_TOKENS=16384
DREAM_MODEL_CONFIG__THINKING_BUDGET_TOKENS=8192
DREAM_MAX_TOOL_ITERATIONS=20
DREAM_HISTORY_TOKEN_LIMIT=16384

# Backup model (optional)
# DREAM_MODEL_CONFIG__FALLBACK_MODEL=gemini-2.5-flash
# DREAM_MODEL_CONFIG__FALLBACK_TRANSPORT=gemini

# Specialist model configs inherit omitted values from DREAM_MODEL_CONFIG
...
# Embedding dimensions should match EMBEDDING_VECTOR_DIMENSIONS
...
6. **Vector Store Issues**
   ...
   - Check `EMBEDDING_VECTOR_DIMENSIONS` matches your embedding model
   - If `VECTOR_STORE_DIMENSIONS` is set explicitly, it must match `EMBEDDING_VECTOR_DIMENSIONS`
