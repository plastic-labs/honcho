...
DREAM_MODEL_CONFIG__MODEL=anthropic/claude-sonnet-4-20250514
...
Instead of an OpenAI-compatible proxy, you can use vendor APIs directly. Each transport picks up its own `LLM_{TRANSPORT}_API_KEY` and `LLM_{TRANSPORT}_BASE_URL`. The default model configs route per feature:
...
Use `transport = "openai"` and point `LLM_OPENAI_BASE_URL` at your endpoint:
...
Set `MODEL_CONFIG__TRANSPORT` and `MODEL_CONFIG__MODEL` for each feature the same way.

Per-module endpoint overrides are also available via `MODEL_CONFIG.overrides`:
...
Default configs use `MODEL_CONFIG.thinking_budget_tokens` tuned for Anthropic models. Non-Anthropic providers don't support extended thinking and will error or silently fail. The [Self-Hosting Guide](./self-hosting#llm-setup) sets these to `0` by default. If you switch to Anthropic models, you can re-enable them:
...
DERIVER_MODEL_CONFIG__TEMPERATURE=  # Optional override (unset by default)
