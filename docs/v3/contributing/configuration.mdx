...
Honcho routes all LLM calls through a single provider type. The recommended setup uses an OpenAI-compatible endpoint (provider `"custom"`) — one base URL, one API key, all features. See the [Self-Hosting Guide](./self-hosting#llm-provider-setup) for quickstart examples.
...
Each feature (deriver, summary, dialectic levels, dream) has its own `PROVIDER` and `MODEL` settings. Set them all to `custom` with your chosen models to use a single endpoint, or use direct vendor keys and let the code defaults route per feature.

<Note>
All Honcho agents (deriver, dialectic, dream) require tool calling support. Verify your models support the OpenAI tool calling format before deploying.
</Note>
...
### Code Defaults (Without Provider Overrides)

If you don't set `PROVIDER` per feature, the code defaults route to:
- **Google** (Gemini) — deriver, summary, dialectic minimal/low
- **Anthropic** (Claude) — dialectic medium/high/max, dream
- **OpenAI** — embeddings (when `EMBED_MESSAGES=true`)

These defaults require 3 separate vendor API keys. For a simpler setup, set all features to `PROVIDER=custom` with a single OpenAI-compatible endpoint — see the [Self-Hosting Guide](./self-hosting#llm-provider-setup).
...
5. **Dialectic Level Configuration**
   ...
   - For non-Anthropic providers (OpenRouter with non-Anthropic models, vLLM, Ollama, Groq, Google, OpenAI), set `THINKING_BUDGET_TOKENS=0` for all components — these providers don't support Anthropic-style extended thinking and will error or silently fail
