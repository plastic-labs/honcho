...
# Milvus-specific
# Default uses Milvus Lite local storage. Use http://localhost:19530 for
# Milvus server, or a Zilliz Cloud URI with VECTOR_STORE_MILVUS_TOKEN.
...
6. **Vector store issues** — For Turbopuffer, set the API key. For Milvus Lite, keep `VECTOR_STORE_MILVUS_URI` pointed at a local `.db` path; for Milvus server or Zilliz Cloud, set the URI and token as needed. Check that `EMBEDDING_VECTOR_DIMENSIONS` matches your embedding model — the startup validator will refuse to boot on a mismatch.
