...
current tiktoken auto-detection behavior. For OpenAI-compatible embedding
models with a different tokenizer, set `tiktoken:<encoding>`, `hf:<repo-id>`,
or `file:/absolute/path/to/tokenizer.json`. Hugging Face and file tokenizers
require installing the optional `honcho[tokenizers]` extra. When the tokenizer
does not match the model's real tokenizer, chunk-size decisions can undercount
tokens and the provider may reject oversized inputs (see issue #827).
