...
```bash
VECTOR_STORE_TYPE=pgvector  # Options: pgvector, turbopuffer, lancedb, milvus
...
# Milvus-specific. Lite is only for single-process development. The API +
# deriver Docker deployment requires a remote Milvus Server or Zilliz Cloud.
...
Milvus is also optional. For a manual Milvus Server or Zilliz Cloud setup, run `uv sync --extra milvus`; for local single-process Milvus Lite development, run `uv sync --extra milvus-lite`. The Docker Compose deployment runs API and deriver in separate containers, so a local Lite database would create isolated, non-durable files. Use a remote Milvus Server or Zilliz Cloud URI and build with `INSTALL_MILVUS=true docker compose up -d --build`. The compose configuration rejects a local Lite URI when `VECTOR_STORE_TYPE=milvus`.
...
[vector_store]
TYPE = "pgvector"
# Milvus Lite is only for single-process development. Docker Compose requires a
# remote Milvus Server or Zilliz Cloud URI.
...
6. **Vector store issues** — For Turbopuffer, set the API key. Install the selected optional extra for LanceDB or Milvus. Milvus Lite requires a local `.db` path and a single-process development setup; Docker Compose requires a remote Milvus Server or Zilliz Cloud URI. Check that `EMBEDDING_VECTOR_DIMENSIONS` matches your embedding model and any existing collection — schema reuse fails before query or upsert when fields, dynamic-field support, index metric, or dimensions are incompatible.
