...
# Optional defaults for the vLLM compat flavor
# These are used when a module opts into transport=openai_compatible and
# compat_provider=vllm without specifying a module-level base_url.
LLM_VLLM_API_KEY=your-vllm-api-key
LLM_VLLM_BASE_URL=http://localhost:8000
```

For non-native OpenAI-compatible endpoints, configure the feature module with:

- `MODEL_CONFIG.transport = "openai_compatible"`
- `MODEL_CONFIG.overrides.base_url` when the module should target a specific endpoint
- `MODEL_CONFIG.overrides.compat_provider` only when you need known endpoint quirks, such as `vllm` or `openrouter`
...
```bash
DIALECTIC_LEVELS__minimal__MODEL_CONFIG__MODEL=gemini/gemini-2.5-flash-lite
...
DERIVER_MODEL_CONFIG__MODEL=gemini/gemini-2.5-flash-lite
...
# DERIVER_MODEL_CONFIG__FALLBACK_MODEL=anthropic/claude-haiku-4-5
...
SUMMARY_MODEL_CONFIG__MODEL=gemini/gemini-2.5-flash
...
# Backup model (optional)
# SUMMARY_MODEL_CONFIG__FALLBACK_MODEL=anthropic/claude-haiku-4-5
...
# Backup model (optional)
# DREAM_MODEL_CONFIG__FALLBACK_MODEL=gemini/gemini-2.5-flash
