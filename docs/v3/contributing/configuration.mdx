...
# HNSW queries scan additional candidates instead of silently
# under-returning when out-of-scope rows consume the initial scan budget.
# Note: pgvector may still stop before reaching top_k if hnsw.max_scan_tuples
# or hnsw.scan_mem_multiplier thresholds are exceeded.
