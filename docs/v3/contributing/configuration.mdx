...
| `anthropic` | Anthropic Claude (direct or custom endpoint) | `LLM_ANTHROPIC_API_KEY` + `LLM_ANTHROPIC_BASE_URL` |
...
LLM_ANTHROPIC_BASE_URL=...   # Optional: custom Anthropic-compatible endpoint
...
LLM_EMBEDDING_MODEL=nomic-embed-text  # Optional, defaults to nomic-embed-text
```
