...
class HonchoStorage(Storage):
    ...
    def search(
        ...
        """
        Search for relevant messages using semantic search and session summaries.

        This method uses Honcho's semantic vector search to find messages most
        relevant to the query, and includes session summaries for context compression.
        This provides better relevance matching than token-limited retrieval.
        ...
        try:
            ...
            # Get session summaries for context compression
            # Summaries are automatically generated by Honcho's background workers
            summaries = self.session.get_summaries()

            # Add summaries to results (provides compressed historical context)
            if summaries.short_summary:
                results.append(
                    {
                        "content": summaries.short_summary.content,
                        "memory": summaries.short_summary.content,
                        "context": f"[Session Summary] {summaries.short_summary.content}",
                        "metadata": {
                            "type": "summary",
                            "summary_type": "short",
                            "created_at": str(summaries.short_summary.created_at),
                        },
                    }
                )
            ...
            if summaries.long_summary:
                results.append(
                    {
                        "content": summaries.long_summary.content,
                        "memory": summaries.long_summary.content,
                        "context": f"[Session Summary] {summaries.long_summary.content}",
                        "metadata": {
                            "type": "summary",
                            "summary_type": "long",
                            "created_at": str(summaries.long_summary.created_at),
                        },
                    }
                )
