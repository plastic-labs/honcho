...
if DB_CONNECTION_URI.startswith("postgresql+psycopg://"):
    DB_CONNECTION_URI = DB_CONNECTION_URI.replace(
...
COLLECTION_NAMES = [f"load_peer_{i + 1}_load_peer_{i + 1}" for i in range(NUM_PEERS)]
...
def create_entities(conn, schema, existing_data):
    ...
    with conn.cursor() as cur:
        ...
        # Create collections (one per peer) if needed
        if existing_data["existing_collections"] < NUM_PEERS:
            # Get existing collection names
            cur.execute(
                f'SELECT name FROM "{schema}"."collections" WHERE workspace_name = %s',
                (WORKSPACE_NAME,),
            )
            existing_collection_names = {row[0] for row in cur.fetchall()}

            for collection_id, collection_name, peer_name in zip(
                COLLECTION_IDS, COLLECTION_NAMES, PEER_NAMES, strict=False
            ):
                if collection_name not in existing_collection_names:
                    cur.execute(
                        f'INSERT INTO "{schema}"."collections" (id, name, peer_name, workspace_name) VALUES (%s, %s, %s, %s)',
                        (collection_id, collection_name, peer_name, WORKSPACE_NAME),
                    )
    ...
    with conn.cursor() as cur:
        for batch_num in range(total_batches):
            ...
            cur.executemany(
                ...
                (id, content, embedding, collection_name, peer_name, workspace_name)
                VALUES (%s, %s, CAST(%s AS vector), %s, %s, %s)''',
                args,
            )
            conn.commit()
