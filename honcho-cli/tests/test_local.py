...
def test_upsert_writes_non_managed_and_preserves_later(tmp_path):
    path = tmp_path / ".env"
    first = managed_env(LocalProfile(name="local"), "sk-new")
    first["DERIVER_MODEL_CONFIG__MODEL"] = "gpt-test"
    upsert_env(path, first)
    upsert_env(path, managed_env(LocalProfile(name="local"), "sk-later"))
    later = path.read_text()
    assert "LLM_OPENAI_API_KEY=sk-later" in later
