...
async def on_reaction_add(reaction, user):
    ...
    example = Example(
        chat_input=chat_input, assessment_dimension=user_state, response=response
    ).with_inputs("chat_input")
    user_state_storage = dict(user.metadata)
    examples = user_state_storage.get("examples", [])
    examples.append(example)
    user_state_storage["examples"] = examples
    user.update(metadata=user_state_storage)
