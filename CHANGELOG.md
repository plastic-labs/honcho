...
- Named `orcarouter` LLM transport: route any text-generation feature through [OrcaRouter](https://www.orcarouter.ai) (an OpenAI-compatible gateway) via `MODEL_CONFIG__TRANSPORT=orcarouter`. It defaults to `https://api.orcarouter.ai/v1`, reads `LLM_ORCAROUTER_API_KEY`, and reuses the OpenAI backend / history adapter. Accepted in the `model/transport` shorthand (`orcarouter/auto`), and `structured_output_mode` is honored as on the `openai` transport.
