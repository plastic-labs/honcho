...
class XRReasoner(BaseReasoner):
    REASONING_TYPE: str = "xr"
    ...
    async def reason(
        ...
        """
        Single-pass reasoning function that performs XR derivations and deductions
        ...
        logger.debug(
            "XR REASONING: message_created_at='%s', new_turns_count=%s",
...
{new_observations.str_no_timestamps()}
    ...
    def base_prompt_render(prompt_type: str) -> str:
        ...
        base_prompt_template = load_prompt_template(
            settings.DERIVER.PROMPTS_BASE_PATH / XRReasoner.REASONING_TYPE, f"{prompt_type}.jinja")
