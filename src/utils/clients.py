...
async def honcho_llm_call_inner(
    ...
    match client:
        case AsyncAnthropic():
            ...
            # Handle thinking + JSON mode combination
            if thinking_budget_tokens:
                anthropic_params["thinking"] = {
                    "type": "enabled",
                    "budget_tokens": thinking_budget_tokens,
                }
                # Note: When extended thinking is enabled, we cannot use prefill with thinking blocks
                # because thinking blocks are generated by the model, not provided by us.
                # We rely on the prompt instructions to ensure JSON output instead of prefilling.
                # The model is capable of following JSON format instructions without prefill.
            else:
                # No thinking enabled, we can use simple text prefill for JSON mode
            ...
            # If response_model is provided, parse and validate the JSON response
            if response_model:
                try:
                    # Validate and repair JSON
                    repaired_json = validate_and_repair_json(content_str)
                    parsed_data = json.loads(repaired_json)

                    # Validate against the response model
                    validated_content = response_model.model_validate(parsed_data)
                    ...
                    logger.error(
                        f"Failed to parse or validate Anthropic response: {e}\nRaw content: {content_str}"
                    )
                    raise
        ...
        case AsyncOpenAI():
            ...
            if provider == "vllm" and response_model:
                ...
                try:
                    ...
                    response_obj = PromptRepresentation()
