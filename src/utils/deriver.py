...
def format_context_for_trace(context: Dict[str, List], include_similarity_scores: bool = False) -> Dict[str, List[Dict[str, Any]]]:
    """
...
def analyze_observation_changes(original_context: dict, revised_observations: dict, 
                               significance_threshold: float, include_details: bool = False):
    """
    ...
    for level in REASONING_LEVELS:
        ...
        original_observations = normalize_observations_for_comparison(original_context.get(level, []))
        revised_observations_list = normalize_observations_for_comparison(revised_observations.get(level, []))
        ...
        if level_changes > 0:
            ...
            if added_observations:
                logger.debug(f"  New observations: {list(added_observations)[:3]}...")  # Show first 3
            if removed_observations:
                logger.debug(f"  Removed observations: {list(removed_observations)[:3]}...")  # Show first 3
        ...
        if include_details and changes_detected is not None:
            changes_detected[level] = {
                ...
                "removed": [{"content": observation, "document_id": "unknown"} for observation in removed_observations],
    ...
    logger.debug(f"Change analysis: {total_changed_observations}/{total_original_observations} observations changed ({change_percentage:.1%})")
    logger.debug(f"Significance threshold: {significance_threshold:.1%} - {'MET' if is_significant else 'NOT MET'}")
...
def find_new_observations(original_context: dict, revised_observations: dict) -> dict:
    ...
    for level in REASONING_LEVELS:
        ...
        for observation in revised_list:
            normalized_observation = extract_observation_content(observation).strip().lower()
