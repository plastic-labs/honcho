...
async def get_session_summaries(
    ...
    metamessage_type = SummaryType.SHORT.value if summary_type == SummaryType.SHORT else SummaryType.LONG.value
...
Provide a {'comprehensive' if summary_type == SummaryType.LONG else 'concise'} summary that captures the key points and context."""
    ...
    try:
        summary = await client.generate(
            ...
            max_tokens=1000 if summary_type == SummaryType.SHORT else 2000,  # Allow longer responses for long summaries
            temperature=0.0,
            use_caching=True
        )
        return summary
...
async def save_summary_metamessage(
    ...
    logger.info(f"Saved {summary_type.name.lower()} summary metamessage for session {session_id} covering {message_count} messages")
    return metamessage

async def get_full_history(db: AsyncSession, session_id: str) -> tuple[str, list[models.Message]]:
    """
...
async def get_summarized_history(
    ...
    messages, latest_summary = await get_messages_since_latest_summary(db, session_id, summary_type)
...
async def get_messages_since_latest_summary(
    ...
    summary = await get_session_summaries(db, session_id, summary_type=summary_type, only_latest=True)
    ...
    if latest_summary is not None:
        messages = await get_messages_since_message(db, session_id, latest_summary.message_id)
