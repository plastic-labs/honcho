...
@pytest.mark.asyncio
async def test_openai_backend_allows_missing_usage() -> None:
    client = Mock()
    client.chat.completions.create = AsyncMock(
        return_value=SimpleNamespace(
            choices=[
...
async def test_openai_backend_rejects_malformed_responses(response: object) -> None:
    client = Mock()
    client.chat.completions.create = AsyncMock(return_value=response)

    backend = OpenAIBackend(client)

    with pytest.raises(ValidationException, match="OpenAI response"):
        await backend.complete(
            model="gpt-4.1",
            messages=[{"role": "user", "content": "Hello"}],
            max_tokens=100,
...
@pytest.mark.asyncio
async def test_openai_structured_response_falls_back_to_json_object() -> None:
    client = Mock()
            ...
            SimpleNamespace(
                choices=[
                    SimpleNamespace(
                        finish_reason="stop",
                        message=SimpleNamespace(
                            content='{"answer": "json object worked"}',
                            tool_calls=[],
                        ),
                    )
                ],
    ...
    assert result.content.answer == "json object worked"
...
@pytest.mark.asyncio
async def test_openai_structured_response_falls_back_to_plain_create() -> None:
    client = Mock()
    client.chat.completions.parse = AsyncMock(
        return_value=SimpleNamespace(choices=[], usage=None)
    )
    ...
    assert isinstance(result.content, StructuredAnswer)
    assert result.content.answer == "plain worked"
    assert client.chat.completions.create.await_count == 3
    first_call, second_call, third_call = client.chat.completions.create.await_args_list
    assert first_call.kwargs["response_format"]["type"] == "json_schema"
    assert second_call.kwargs["response_format"] == {"type": "json_object"}
    assert "response_format" not in third_call.kwargs
