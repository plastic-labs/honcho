    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "classification": {
                ...
                "molecular_rate": round(self.molecular_count / self.proposition_count, 3) if self.proposition_count > 0 else 0,
            },
            "issues": {
...
class MolecularJudge:
    ...
    async def _call_llm(self, system: str, user: str, response_schema: dict[str, Any]) -> dict[str, Any]:
            ...
            "properties": {
                "analyses": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "ambiguities": {
                                ...
                                "items": {
                                    "type": "object",
                                    "properties": {
                                        "type": {"type": "string", "enum": [t.value for t in AmbiguityType]},
                                        "span": {"type": "string"},
                        ...
                        "required": ["index", "ambiguities", "primary_subject", "requires_context"],
                    },
                }
            },
            "required": ["analyses"],
    ...
    async def analyze_decontextuality(
                        ...
                        "properties": {
                            ...
                            "issues": {
                                ...
                                "items": {
                                    "type": "object",
                                    "properties": {
                                        "type": {"type": "string", "enum": [t.value for t in DecontextualityIssue]},
        ...
        for item in result.get("analyses", []):
            ...
            if 0 <= idx < len(analyses):
                ...
                a.suggested_decontextualization = item.get("suggested_decontextualization", "")
            ...
            "properties": {
                "analyses": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "issues": {
                                ...
                                "items": {
                                    "type": "object",
                                    "properties": {
                                        "type": {"type": "string", "enum": [t.value for t in MinimalityIssue]},
                                        "description": {"type": "string"},
                                    },
                                },
                            },
                        ...
                        "required": ["index", "score", "is_minimal", "token_count", "claim_count"],
                    },
                }
            },
            "required": ["analyses"],
    ...
    async def evaluate(
        ...
        logger.info(f"Evaluating {len(propositions)} propositions (3-stage molecular pipeline)...")
        ...
        for i, prop in enumerate(propositions):
            analysis = MolecularAnalysis(
                proposition=prop,
                ambiguity=ambiguity_results[i] if i < len(ambiguity_results) else AmbiguityAnalysis(proposition=prop),
                decontextuality=decontext_results[i] if i < len(decontext_results) else DecontextualityAnalysis(proposition=prop, score=0.5, is_standalone=False),
                minimality=minimal_results[i] if i < len(minimal_results) else MinimalityAnalysis(proposition=prop, score=0.5, is_minimal=False),
            )
        ...
        # Compute averages
        report.avg_decontextuality = sum(a.decontextuality.score for a in analyses) / len(analyses)
        report.avg_minimality = sum(a.minimality.score for a in analyses) / len(analyses)
        ...
        for a in analyses:
            for amb_type, _, _ in a.ambiguity.ambiguities:
                report.ambiguity_distribution[amb_type.value] = report.ambiguity_distribution.get(amb_type.value, 0) + 1
            for issue_type, _ in a.decontextuality.issues:
                report.decontextuality_issues[issue_type.value] = report.decontextuality_issues.get(issue_type.value, 0) + 1
            for issue_type, _ in a.minimality.issues:
                report.minimality_issues[issue_type.value] = report.minimality_issues.get(issue_type.value, 0) + 1
...
def print_report(report: MolecularReport) -> None:
    ...
    print("=" * 70)
    print(f"Peer: {report.peer_name} | Propositions: {report.proposition_count} | Messages: {report.source_message_count}")
    ...
    print(f"  ✓ Molecular:    {report.molecular_count:3d} ({report.molecular_count/total*100:5.1f}%)")
    print(f"  ⚠ Too Atomic:   {report.too_atomic_count:3d} ({report.too_atomic_count/total*100:5.1f}%)")
    print(f"  ⚠ Too Verbose:  {report.too_verbose_count:3d} ({report.too_verbose_count/total*100:5.1f}%)")
    print(f"  ~ Borderline:   {report.borderline_count:3d} ({report.borderline_count/total*100:5.1f}%)")
    
    # Show top issues
    if report.ambiguity_distribution:
        print("\nTop Ambiguity Types:")
        for k, v in sorted(report.ambiguity_distribution.items(), key=lambda x: -x[1])[:3]:
            print(f"  - {k}: {v}")
    
    if report.decontextuality_issues:
        print("\nTop Decontextuality Issues:")
        for k, v in sorted(report.decontextuality_issues.items(), key=lambda x: -x[1])[:3]:
            print(f"  - {k}: {v}")
    ...
    if too_atomic:
        ...
        for a in too_atomic[:2]:
            ...
            if a.decontextuality.suggested_decontextualization:
                print(f'    → Suggest: "{a.decontextuality.suggested_decontextualization[:60]}..."')
...
async def main():
    parser = argparse.ArgumentParser(description="MolecularBench - Molecular Facts Evaluation")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("tests/bench/molecular_results"))
    ...
    if args.sample and 0 < args.sample < 1:
        all_traces = random.sample(all_traces, max(1, int(len(all_traces) * args.sample)))
    if args.limit:
