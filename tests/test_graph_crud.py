...
endpoints against a per-test PostgreSQL database.
"""
...
async def test_delete_edge(
    ...
    result = await db_session.execute(select(models.Edge).where(models.Edge.id == edge["id"]))
...
@pytest.mark.asyncio
async def test_context_creation_and_member_management(
    client: TestClient,
    graph_memory_setup: dict[str, Any],
    db_session: AsyncSession,
) -> None:
    ...
    # Deleting all members effectively removes the context from queries.
    result = await db_session.execute(
        select(func.count()).select_from(models.ContextIndex).where(
            models.ContextIndex.workspace_name == workspace.name,
            models.ContextIndex.context_name == ctx_name,
        )
    )
...
async def test_pin_and_unpin_observation(
    ...
    resp = client.delete(f"/v3/workspaces/{workspace.name}/graph-memory/observations/{obs_id}/pin")
    assert resp.status_code == 200
...
async def test_verify_observation(
    ...
    resp = client.post(f"/v3/workspaces/{workspace.name}/graph-memory/observations/{obs_id}/verify")
    assert resp.status_code == 200
...
async def test_rehydrate_cold_observation(
    ...
    resp = client.post(f"/v3/workspaces/{workspace.name}/graph-memory/rehydrate/{obs_id}")
