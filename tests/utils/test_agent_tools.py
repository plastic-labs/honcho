...
async def tool_test_data(
    ...
    for i, content in enumerate(
        ...
        doc = models.Document(
            ...
            observed=peer2.name,
            content=content,
            embedding=[0.1 * (i + 1)] * settings.EMBEDDING.VECTOR_DIMENSIONS,
            session_name=session.name,
            level="explicit",
            metadata={
...
class TestDeleteObservations:
    ...
    async def test_delete_batch_emits_levels_for_successful_only(
        ...
        deductive_doc = models.Document(
            ...
            embedding=[0.42] * settings.EMBEDDING.VECTOR_DIMENSIONS,
            session_name=session.name,
            level="deductive",
            metadata={},
        )
        inductive_doc = models.Document(
            ...
            embedding=[0.43] * settings.EMBEDDING.VECTOR_DIMENSIONS,
            session_name=session.name,
            level="inductive",
            metadata={},
        )
