"""Behavior of the trigger-maintained queue_item_batches and the claim that reads it.

The batch table holds one row per work unit that has anything unprocessed in the
...
from src.crud.deriver import batch_threshold_clause, unclaimed_work_unit_clause
...
async def _read_batch_row(db: AsyncSession, work_unit_key: str) -> Row[Any] | None:
    ...
    result = await db.execute(
        select(
            models.QueueItemBatch.tenant_id,
            models.QueueItemBatch.task_type,
            models.QueueItemBatch.pending_count,
            models.QueueItemBatch.total_tokens,
            models.QueueItemBatch.oldest_created_at,
        ).where(models.QueueItemBatch.work_unit_key == work_unit_key)
    )
...
def _claim_candidate_query(limit: int) -> Select[tuple[str]]:
    ...
    query = (
        select(models.QueueItemBatch.work_unit_key)
...
@pytest.mark.asyncio
class TestEnqueueMaintainsTheBatches:
    """The insert direction: one incremented row per work unit."""

    async def test_enqueued_items_aggregate_into_one_batch_row(
        self,
        db_session: AsyncSession,
        ...
        assert batch_row.total_tokens == 23
        assert batch_row.task_type == "representation"
        assert batch_row.tenant_id == DEFAULT_TENANT_ID
        assert batch_row.oldest_created_at == min(
    ...
    async def test_a_later_item_with_an_older_timestamp_moves_oldest_back(
        ...
        assert batch_row.total_tokens == 7
        assert batch_row.oldest_created_at == backdated_items[0].created_at
    ...
    async def test_tenantless_queue_rows_still_sum_their_tokens(
        ...
        assert batch_row.total_tokens == 15
...
@pytest.mark.asyncio
class TestCompletionRecomputesTheBatches:
    ...
    async def test_completing_the_oldest_item_shrinks_the_row(
        ...
        assert batch_row is not None
        assert batch_row.pending_count == 2
        assert batch_row.total_tokens == 18
        assert batch_row.oldest_created_at == queue_items[1].created_at
    ...
    async def test_reopening_a_completed_item_brings_the_row_back(
        ...
        assert batch_row.total_tokens == 9


@pytest.mark.asyncio
class TestDeletionRecomputesTheBatches:
...
class TestNonRepresentationUnits:
    ...
    async def test_a_webhook_unit_carries_no_tokens_and_is_claimable(
        ...
        assert batch_row is not None
        assert batch_row.task_type == "webhook"
        assert batch_row.total_tokens == 0
...
class TestTriggerEffectsAreTransactional:
    ...
    async def test_a_rolled_back_enqueue_leaves_no_batch_row(
        self,
...
@pytest.mark.asyncio
class TestClaimReadsTheBatches:
...
class TestConcurrentClaimersTakeDisjointUnits:
    """SKIP LOCKED on the batch rows is what keeps two derivers from colliding."""
...
@pytest.mark.asyncio
class TestMetricsReadTheSameBatches:
    ...
    async def test_metrics_return_to_zero_once_the_queue_drains(
        ...
        """With every batch row gone the gauges read zero, not a stale total."""
