"""Interleavings that made the batch aggregate disagree with the queue behind it.

Two writers on one work unit are the only way the aggregate can drift. The
enqueue path increments the batch row it holds a lock on; a completion
...
NOT EXISTS subquery. A live unit would then have no batch row at all, and the
claim would never offer it again.

Each interleaving below was reproduced against the unfixed triggers. The
recompute now locks the batch row first, so every statement after the wait
...
async def _committed_message_priced_at(
    ...
    already-processed unit keeps it out of the batch table entirely.
    """
...
async def _enqueue_pending_item(
    ...
    *,
    ...
    One statement, so the row-level insert trigger's batch upsert — and the
...
class TestACompletionRacingAnEnqueue:
    ...
    async def test_the_delete_guard_keeps_a_unit_whose_next_item_arrived_mid_recompute(
        ...
        The enqueue holds the batch row's lock uncommitted, so the completion's
        ...
        subquery has stopped being true. The unit lost its batch row while it
        ...
        async with sessions() as enqueuing, sessions() as completing:
            ...
            try:
                ...
                assert not completion.done(), (
                    "the recompute must block on the enqueue's batch row lock — "
