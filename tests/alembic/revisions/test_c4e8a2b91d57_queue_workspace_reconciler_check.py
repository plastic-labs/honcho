...
def verify_queue_workspace_reconciler_check(verifier: MigrationVerifier) -> None:
    ...
    # convalidated flips. That the flag IS set is also the receipt that the
    # upgrade's autocommit blocks did their job: without them alembic's single
    # transaction would still hold ADD CONSTRAINT's ACCESS EXCLUSIVE here, and
    # the validation scan would have run under the lock it exists to avoid.
    # endregion
    ...
    assert violating_rows == 0, (
        f"{violating_rows} row(s) violate the lane invariant VALIDATE CONSTRAINT "
        + "is supposed to have proven clean"
    )
