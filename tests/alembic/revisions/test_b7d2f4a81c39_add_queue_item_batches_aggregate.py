"""Hooks for revision b7d2f4a81c39 (add_queue_item_batches_aggregate)."""
...
def prepare_add_queue_item_batches_aggregate(verifier: MigrationVerifier) -> None:
    """Seed a queue that already holds pending work before upgrading."""
    verifier.assert_table_exists("queue_item_batches", exists=False)
...
def verify_add_queue_item_batches_aggregate(verifier: MigrationVerifier) -> None:
    """Assert the aggregate landed and the pending queue was backfilled exactly."""
    conn = verifier.conn
    schema = verifier.schema

    verifier.assert_table_exists("queue_item_batches")
    verifier.assert_indexes_exist(list(INDEXES))

    def batch_row(work_unit_key: str):
        return conn.execute(
            text(
                "SELECT tenant_id, task_type, pending_count, total_tokens, "
                + f'oldest_created_at FROM "{schema}"."queue_item_batches" '
    ...
    total_rows = conn.execute(
        text(f'SELECT COUNT(*) FROM "{schema}"."queue_item_batches"')
    ).scalar()
    assert total_rows == 4, f"expected 4 backfilled units, found {total_rows}"
    assert batch_row(DRAINED_UNIT) is None, "a drained unit must not be backfilled"

    tenant_unit = batch_row(TENANT_UNIT)
    ...
    tenantless_unit = batch_row(TENANTLESS_UNIT)
    ...
    webhook_unit = batch_row(WEBHOOK_UNIT)
    ...
    assert batch_row(WEBHOOK_UNIT) is None, (
        "completing a unit's last pending item must delete its batch row"
    )
