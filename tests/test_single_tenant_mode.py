...
a value out of a cloud config gets unchanged single-tenant behavior, never a
half-state. Parametrized over each subordinate and over all of them at once, with
the same assertions every time: the values are varied on purpose, because the claim
under test is that they do not matter.
"""
...
_STRAY_SECRET = "copied-from-a-cloud-config"
...
_SUBORDINATES: dict[str, tuple[Subordinate, ...]] = {
    "skip_rls_assert": (_skip_rls_assert,),
    "service_connection_uri": (_service_connection_uri,),
    "transaction_pooler": (_transaction_pooler,),
    "tenant_api_secret": (_tenant_api_secret,),
    "all_at_once": (
        _skip_rls_assert,
...
@pytest.fixture(params=list(_SUBORDINATES), ids=list(_SUBORDINATES))
def stray_subordinates(
    request: pytest.FixtureRequest, monkeypatch: pytest.MonkeyPatch
) -> str:
    """Flag off, with one (or every) subordinate set to a value flag-on would act on."""
    monkeypatch.setattr(settings, "MULTI_TENANT", False)
    case = cast(str, request.param)
    for apply in _SUBORDINATES[case]:
        apply(monkeypatch)
    return case


@pytest.mark.usefixtures("stray_subordinates")
@pytest.mark.asyncio
async def test_isolation_validator_is_a_no_op() -> None:
    # A transaction-mode pooler or a missing service role are boot refusals flag-on;
    # flag-off the validator returns before reading either, or the engine.
...
async def test_tenant_bearing_token_binds_nothing(
    ...
    # The dedicated-instance case: a control plane that mints tenant claims for every
    # tenant talks to a flag-off instance. The claim survives on the params and is
    # otherwise ignored — rows keep stamping the default tenant.
