...
class TestEnqueueFunction:
    ...
    # Helper methods
    def create_sample_payload(self, workspace_name="test_workspace", session_name: Optional[str] = "test_session", peer_name="test_peer", count=1):
    ...
    async def test_none_session_with_observe_me_true(self, mock_tracked_db, db_session, sample_data):
    ...
    @pytest.mark.asyncio
    @patch('src.routers.messages.tracked_db')
    async def test_none_session_with_observe_me_false(self, mock_tracked_db, db_session, sample_data, caplog):
        ...
        assert f"Peer {test_peer.name} has observe_me=False, skipping enqueue" in caplog.text

    # SESSION MESSAGES
    @pytest.mark.asyncio
    @patch('src.routers.messages.tracked_db')
    async def test_session_with_deriver_disabled(self, mock_tracked_db, db_session, sample_data, caplog):
        ...
        # Should not create any queue items
        assert final_count == initial_count
        assert f"Deriver is disabled for session {test_session.name}, skipping enqueue" in caplog.text

    @pytest.mark.asyncio
    @patch('src.routers.messages.tracked_db')
    async def test_session_normal_processing_single_peer(self, mock_tracked_db, db_session, sample_data):
        mock_tracked_db.return_value.__aenter__.return_value = db_session

        test_workspace, test_peer = sample_data

        test_session = await crud.get_or_create_session(db_session, schemas.SessionCreate(name=str(generate_nanoid()), peer_names={test_peer.name: schemas.SessionPeerConfig(observe_me=True)}), test_workspace.name)
        await db_session.commit()
        ...
        # Should create 2 queue items: 1 representation + 1 summary
    ...
    @pytest.mark.asyncio
    @patch('src.routers.messages.tracked_db')
    async def test_session_with_multiple_peers_none_observe_others(self, mock_tracked_db, db_session, sample_data):
        ...
        test_session = await crud.get_or_create_session(db_session, schemas.SessionCreate(name=str(generate_nanoid()), peer_names={test_peer1.name: schemas.SessionPeerConfig(), test_peer2.name: schemas.SessionPeerConfig()}), test_workspace.name)
        ...
        # Should create NUM_MESSAGES * 2queue items: 
        # 1 representation for sender and 1 summary for each message
    ...
    @pytest.mark.asyncio
    @patch('src.routers.messages.tracked_db')
    async def test_session_with_multiple_peers_all_observe_others(self, mock_tracked_db, db_session, sample_data):
        """Test session processing with multiple peers where some observe others"""
        mock_tracked_db.return_value.__aenter__.return_value = db_session
        ...
        test_session = await crud.get_or_create_session(db_session, schemas.SessionCreate(name=str(generate_nanoid()), peer_names={test_peer1.name: schemas.SessionPeerConfig(), test_peer2.name: schemas.SessionPeerConfig(observe_others=True)}), test_workspace.name)
        await db_session.commit()
        ...
        # Should create NUM_MESSAGES * 3 queue items: 
        # 1 representation for sender, 1 local representation for observer, and 1 summary for each message
        assert final_count - initial_count == NUM_MESSAGES * 3
    ...
    @pytest.mark.asyncio
    @patch('src.routers.messages.tracked_db')
    async def test_session_with_multiple_peers_some_observe_others(self, mock_tracked_db, db_session, sample_data):
        """Test session processing with multiple peers where some observe others"""
        mock_tracked_db.return_value.__aenter__.return_value = db_session
        ...
        # Create session
        test_session = await crud.get_or_create_session(db_session, schemas.SessionCreate(name=str(generate_nanoid()), peer_names={test_peer1.name: schemas.SessionPeerConfig(observe_me=True), observing_peer.name: schemas.SessionPeerConfig(observe_others=True), unobserving_peer.name: schemas.SessionPeerConfig()}), test_workspace.name)
        await db_session.commit()
        ...
        # Should create NUM_MESSAGES * 3 queue items: 
        # 1 representation for sender, 1 local representation for 1 peer observer, and 1 summary for each message
        assert final_count - initial_count == NUM_MESSAGES * 3
        ...
        assert unobserving_peer.name not in [item.payload.get("target_name") for item in queue_items]


    @pytest.mark.asyncio
    @patch('src.routers.messages.tracked_db')
    async def test_session_peer_config_overrides_peer_config(self, mock_tracked_db, db_session, sample_data):
        ...
        # Create session
        test_session = await crud.get_or_create_session(db_session, schemas.SessionCreate(name=str(generate_nanoid()), peer_names={test_peer.name: schemas.SessionPeerConfig(observe_me=False)}), test_workspace.name)
        ...
        # Should create 1 queue item (summary)
        assert final_count - initial_count == 1
        ...
        assert queue_items[0].payload["task_type"] == "summary"


    @pytest.mark.asyncio
    @patch('src.routers.messages.tracked_db')
    async def test_session_summary_flag_controls_summary_creation(self, mock_tracked_db, db_session, sample_data):
        mock_tracked_db.return_value.__aenter__.return_value = db_session
        """Test that session summarize flag controls summary queue item creation"""
        test_workspace, test_peer = sample_data
        
        # Create session with summarize=False
        test_session = models.Session(
            workspace_name=test_workspace.name,
            name=str(generate_nanoid()),
            feature_flags={"summarize": False}
        )
        db_session.add(test_session)
        ...
        # Verify it's a representation item
        result = await db_session.execute(
            select(QueueItem).where(QueueItem.session_id == test_session.id)
        )
        queue_items = result.scalars().all()
        ...
        task_types = {item.payload["task_type"] for item in queue_items}
        assert "representation" in task_types
        assert "summary" not in task_types


    @pytest.mark.asyncio
    @patch('src.routers.messages.tracked_db')
    async def test_multi_sender_scenario(self, mock_tracked_db, db_session, sample_data):
        ...
        test_session = await crud.get_or_create_session(db_session, schemas.SessionCreate(name=str(generate_nanoid()), peer_names={test_peer1.name: schemas.SessionPeerConfig(), additional_sender_peer.name: schemas.SessionPeerConfig(), observer_peer.name: schemas.SessionPeerConfig(observe_others=True)}), test_workspace.name)
        await db_session.commit()
        ...
        # Should create 6 queue items: 
        # 1 summary for each message (2)
